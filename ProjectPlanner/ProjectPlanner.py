    ...
    while incomplete:
        for task_number in incomplete:
            ...
            if task.prerequisites.issubset(completed):
                ...
                for prerequ_number in task.prerequisites:
                    prereq_end_day = start_days[prerequ_number] + tasks[prerequ_number].duration
...
def draw_chart(tasks, canvas, row_height=40, title_width=300,line_height=40,
    ...
    for task_number in start_days:
        task = tasks[task_number]
        canvas.create_text(title_indent, y + row_height/2, text=task.title
                            , anchor=tkinter.W, font=("Helvetica", font_size))
...
tasks = read_tasks("ProjectPlanner.csv")
